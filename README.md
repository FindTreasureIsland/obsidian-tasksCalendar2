...
## Updates & Changes (2025-10)

Enhancements to interaction and note handling:

- Clicking a task or date opens the note in an Obsidian popout window, centered horizontally and vertically.
- The popout window hides the note title bar and tab bar; the main window remains unchanged.
- New daily notes use the core “Daily Notes” plugin settings (folder, date format, optional template).
- Hover preview is removed; click to view/edit instead.
- New non-daily notes (where applicable) respect Obsidian’s “Default location for new notes”.

> Note: OS-level window title bars cannot be hidden via script; this change hides only Obsidian UI elements inside the popout.
...
await dv.view("tasksCalendar", {
  ...
  options: "style1",
  // dailyNoteFolder: "Your/Daily/Notes/Folder" // Optional: affects displayed path; actual creation uses Daily Notes settings
});
```
````

Interaction:
- Month/Week views: click the day label (top-left of each cell) to create/open the daily note in the Daily Notes folder using its configured format/template.
- List/Grid tasks: click a task to open the related note in a popout window.
- Hover preview is disabled; use click to open.

Parameters (compatible with original):
- `pages`: Dataview source (string or `dv.pages(...)` expression).
- `view`: default view (`list` | `month` | `week`).
- `firstDayOfWeek`: week start (`"1"` or `"0"`).
- `options`: style classes (e.g., `style1`).
- `dailyNoteFolder` (optional): used for populating day cell data-path; creation location is governed by the Daily Notes plugin.

Troubleshooting:
- Popout not centered: some platforms constrain window movement; try reopening or review OS window management settings.
- Daily note not created in expected folder: enable/configure the core “Daily Notes” plugin (folder, date format, template).
- Couldn’t hide title/tab bars: only the popout hides them; the main window is unaffected. If your theme uses different class names, provide feedback to add compatibility.
